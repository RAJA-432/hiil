...
from veda_engine.tools.workspace import glob, grep, read_text_resource, search_resources
...
async def test_glob_finds_py_files():
    ...
    assert "webui.py" in results or "veda_engine.py" in results
...
async def test_grep_no_match():
    with tempfile.TemporaryDirectory() as td:
        ...
        with patch("veda_engine.tools.workspace.WORKSPACE_ROOT", tmp):
