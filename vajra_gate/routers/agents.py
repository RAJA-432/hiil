...
async def add_agent_route(request: Request, agent_id: str, user: str = Depends(get_current_user)):
    chat = await _require_chat(request)
    runner = chat.get_agent(agent_id)
    if runner is None:
        raise HTTPException(status_code=404, detail=f"Agent '{agent_id}' not found")
...
async def run_agent(request: Request, agent_id: str, user: str = Depends(get_current_user)):
    chat = await _require_chat(request)
    runner = chat.get_agent(agent_id)
    if runner is None:
        raise HTTPException(status_code=404, detail=f"Agent '{agent_id}' not found")
